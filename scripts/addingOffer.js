...
async function main() {
  ...
  const offer = await OFFER.deploy(1, teacher2.address, 10 * 10 ** 9, '20PLN', 'Some desc', 1, 'kupon');
  await offer.deployed();

  const savedOffer = await offer.getOfferStats()
  const value = await offer.getHiddenValue()

  console.log('offer', savedOffer, value)
}
